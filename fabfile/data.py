...
def create_tables():
    ...
    models.UnemploymentData.create_table()
    ...
    with open('data/unemployment.csv') as f:
        ...
        for row in unemployment_reader:
            ...
            results = models.Result.select(models.Result.id).where(
                models.Result.fipscode == full_fips,
                models.Result.officename == 'President'
            )

            for result in results:
                print(full_fips)
                models.UnemploymentData.create(fipscode=full_fips, unemployment_rate=rate, unemployment_id=result.id)
